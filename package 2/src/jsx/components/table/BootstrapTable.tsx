...
// Define the type for the examTopper data
interface ExamTopper {
  id: string;
  color: string;
  status: string;
  inputchecked?: boolean; // Optional, initially undefined, will be toggled
}

const examTopper: ExamTopper[] = [
...
const BootstrapTable = () => {
  // State to store the data for checked items and if all are unchecked
  const [checked, setChecked] = useState<ExamTopper[]>(examTopper);
  const [unchecked, setUnChecked] = useState<boolean>(true);

  // Handle toggling individual checkbox
  ...
  // Handle toggling "check all" checkbox
  const handleCheckedAll = (value: boolean): void => {
    const temp = checked.map((data) => {
                  ...
                  {checked.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <div className="d-flex align-items-center">
                          <i
                            className={`fa fa-circle text-${item.color} me-1`}
                          ></i>{" "}
                          {item.status}
                        </div>
                      </td>
