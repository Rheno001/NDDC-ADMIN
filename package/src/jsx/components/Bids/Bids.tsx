...
const statusBadge: Record<string, { badge: string; text: string }> = {
    ...
    "Draft": { badge: "badge-secondary", text: "Draft" },
    "SUBMITTED_FOR_APPROVAL": { badge: "badge-info", text: "Submitted" },
...
const Bids = () => {
    ...
    const fetchBids = useCallback(async (refNum?: string, status?: string, page?: number) => {
        ...
        try {
            ...
            if (activeRef) {
                // If the search looks like a reference number (TND-...), send it as tenderReferenceNumber
                // Otherwise, send it as the title to allow searching by bid name
                if (activeRef.toUpperCase().includes("TND-")) {
                    params.append('tenderReferenceNumber', activeRef);
                } else {
                    params.append('title', activeRef);
                }
            }
                ...
                <div className="col-xl-12">
                    <div className="row">
                            ...
                            <div className="page-title flex-wrap">
                                <form className="input-group search-area mb-md-0 mb-3" onSubmit={handleSearch}>
                                    <input
                                        ...
                                        placeholder="Search by Title or Ref (e.g. TND-2026...)"
                                        value={searchTerm}
                                        onChange={(e) => {
                                ...
                                <div className="d-flex align-items-center">
                                    ...
                                    >
                                        <Dropdown.Toggle as="div" className="drop-select-btn" style={{ width: "100%", paddingRight: "30px" }}>
                                            {statusBadge[statusFilter]?.text ?? (statusFilter === "All Statuses" ? "All Statuses" : statusFilter)}
                                        </Dropdown.Toggle>
                                        <Dropdown.Menu align="end">
                                            <Dropdown.Item eventKey="All Statuses">All</Dropdown.Item>
                                            <Dropdown.Item eventKey="DRAFT">Draft</Dropdown.Item>
                                            <Dropdown.Item eventKey="SUBMITTED_FOR_APPROVAL">Submitted</Dropdown.Item>
                        ...
                        <CreateBidModal
                            ref={modalRef}
                            onBidCreated={() => {
                                console.log("ON BID CREATED CALLED. Resetting filters to All Statuses specifically.");
                                setSearchTerm("");
                                setStatusFilter("All Statuses");
                                setPageIndex(0);
                                fetchBids("", "All Statuses", 0);
