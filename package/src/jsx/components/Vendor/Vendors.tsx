  ...
  const columns = useMemo<ColumnDef<VendorRow>[]>(
    () => [
      ...
      },
      {
        ...
        accessorKey: "id",
        cell: ({ row }: { row: Row<VendorRow> }) => (
          <span className="font-w500">#{String(row.original.id || "").slice(-6).toUpperCase()}</span>
        ),
      },
      {
        header: "Vendor Name",
              ...
              <Dropdown className="custom-dropdown">
                ...
                >
                  <Dropdown.Item onClick={() => {
                    navigate(`/vendor-documents/${item.id}`);
                  }}>
                    View documents
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => {
                    navigate("/vendor-details", { state: { vendorId: item.id } });
                  }}>
                    View profile
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => {
                    navigate(`/vendor-bids/${item.id}`);
                  }}>
                    View Bids
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => {
                    navigate(`/vendor-evaluation/${item.id}`);
                  }}>
                    View Evaluation
                  </Dropdown.Item>
                  <Dropdown.Item className="text-danger" onClick={() => {
                    toast.warning(`Suspending vendor: ${item.companyName}`);
                  }}>
                    Suspend Vendor
                  </Dropdown.Item>
                </Dropdown.Menu>
